// Adapted from Cloudflare's remote-mcp-github-oauth demo (MIT License, Copyright (c) 2025 Cloudflare, Inc.).
// https://github.com/cloudflare/ai/tree/main/demos/remote-mcp-github-oauth — see THIRD_PARTY_NOTICES.md.
...
app.get("/authorize", async (c) => {
	...
	return renderApprovalDialog(c.req.raw, {
		...
		csrfToken,
		server: {
			description: "Private OAuth gateway for Tencent CloudBase Hosted MCP.",
			logo: "https://avatars.githubusercontent.com/u/314135?s=200&v=4",
			name: "CloudBase MCP Gateway",
		},
		setCookie,
		state: { oauthReqInfo },
	});
 ...
 * It exchanges the temporary code for an access token, then stores only the
 * minimum user metadata required for authorization in the MCP token. It ends by redirecting the client back to _its_ callback URL
 *
...
app.get("/callback", async (c) => {
	...
	// This is a private MCP gateway: only the configured GitHub account may complete authorization.
	if (!env.ALLOWED_GITHUB_LOGIN || login !== env.ALLOWED_GITHUB_LOGIN) {
		return c.text("Forbidden: this MCP server is private.", 403);
	}
